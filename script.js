...
function dropCanvas(ev) {
    ...
    if (ev.target.getAttribute('id').charAt(9) == 'p' && newBtn.getAttribute('id').charAt(0) == 'c') {
        ...
        if (new_index != old_index) { //만약 자기자신의 버튼에 넣을경우 에러가 발생해서 방지하였습니다-0413추가
            var direction = (new_index > old_index) ? -1 : 1;
            while ((new_index + direction) != old_index + !!(direction + 1)) { /*앞블럭에서 뒤블럭로 드래그 할 경우 해당 블럭의 앞까지만 이동하는 현상-0413수정*/
